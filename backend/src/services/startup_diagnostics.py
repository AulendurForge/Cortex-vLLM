...
ERROR_PATTERNS = [
    {
        "pattern": r"LocalEntryNotFoundError.*Cannot find.*cached snapshot.*outgoing traffic.*disabled",
        "title": "Tokenizer Not Available in Offline Mode",
        "message": "vLLM is trying to download a tokenizer but offline mode is enabled. The tokenizer is not cached locally.",
        "fixes": [
            "Use 'HF Config Path' in model settings to point to a local directory containing tokenizer files",
            "Pre-cache the tokenizer while online: run 'huggingface-cli download <tokenizer_repo_id>'",
            "Place tokenizer files (tokenizer.json, tokenizer_config.json, vocab.json) in the model directory and set hf_config_path to that path",
            "If using a GGUF model, use the base model's tokenizer directory as hf_config_path",
        ],
        "error_type": "model",
        "severity": "error",
    },
    {
        "pattern": r"assert.*HF_HUB_OFFLINE|huggingface.*offline.*mode.*cannot.*download",
        "title": "HuggingFace Offline Mode Download Blocked",
        "message": "A required file cannot be downloaded because HuggingFace Hub is in offline mode.",
        "fixes": [
            "Pre-cache all required files while online before deploying in offline mode",
            "Use local paths instead of HuggingFace repository IDs",
            "Set hf_config_path to a local directory containing required configuration files",
        ],
        "error_type": "model",
        "severity": "error",
    },
    ...
    {
        "pattern": r"NCCL.*timeout|nccl.*timeout|Watchdog caught.*NCCL",
        "title": "NCCL Communication Timeout",
        "message": "Multi-GPU communication via NCCL timed out. This usually indicates GPU interconnect issues or one GPU became unresponsive.",
        "fixes": [
            "Check GPU interconnect health: run 'nvidia-smi topo -m' on host",
            "Verify all GPUs are accessible: run 'nvidia-smi' on host",
            "Try setting NCCL_P2P_DISABLE=1 if using PCIe (not NVLink)",
            "Increase NCCL_TIMEOUT in custom environment variables",
            "Check for other processes holding GPU resources",
            "Enable NCCL debugging: set NCCL_DEBUG=INFO or TRACE",
        ],
        "error_type": "multi_gpu",
        "severity": "error",
    },
    {
        "pattern": r"NCCL.*error.*communicator|ncclInternalError|ncclSystemError",
        "title": "NCCL Communication Error",
        "message": "Multi-GPU NCCL communication failed. This can be caused by GPU driver issues, network problems, or incompatible configurations.",
        "fixes": [
            "Check NVIDIA driver compatibility across GPUs",
            "Set NCCL_DEBUG=INFO to see detailed error messages",
            "Try NCCL_P2P_DISABLE=1 to disable peer-to-peer communication",
            "Verify all GPUs have the same driver version",
            "Check for GPU memory exhaustion on any GPU in the group",
            "Restart Docker to clear stale NCCL state",
        ],
        "error_type": "multi_gpu",
        "severity": "error",
    },
    {
        "pattern": r"Error in memory profiling.*other processes.*release GPU memory",
        "title": "GPU Memory Changed During Profiling",
        "message": "Another process released GPU memory while vLLM was profiling available VRAM, causing inconsistent memory calculation.",
        "fixes": [
            "Ensure no other GPU processes during vLLM startup",
            "Use --kv-cache-memory to specify exact KV cache size in bytes",
            "Restart any other GPU processes after vLLM is running",
            "Consider isolating vLLM on dedicated GPU(s)",
        ],
        "error_type": "memory",
        "severity": "error",
    },
